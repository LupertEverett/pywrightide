...
class MainWindowStatusBar(QStatusBar):
    ...
    def __init__(self, parent=None):
        ...
        self._selection_info_separator = StatusBarSeparator()
        self._line_col_info_label = QLabel()
        self._line_col_info_separator = StatusBarSeparator()
        ...
        self.addPermanentWidget(self._selection_info_separator)
        self.addPermanentWidget(self._line_col_info_label)
        self.addPermanentWidget(self._line_col_info_separator)
    ...
    def set_selection_length_info(self, selection_length: int):
        if selection_length > 0:
            self._selection_info_separator.setHidden(False)
            self._selection_length_info_label.setText("{} characters selected".format(selection_length))
        else:
            self._selection_info_separator.setHidden(True)
    ...
    def set_cursor_position_info(self, line: int, col: int):
        ...
        if line > 0 and col > 0:
            self._line_col_info_separator.setHidden(False)
            self._line_col_info_label.setText("Ln: {}, Col: {}".format(line, col))
        else:
            self._line_col_info_separator.setHidden(True)
            self._line_col_info_label.setText("")


class StatusBarSeparator(QFrame):
    """Literally just a vertical line. Why QStatusBar simply doesn't have an addSeparator() method is anyone's guess."""
    def __init__(self, parent=None):
        super().__init__()
