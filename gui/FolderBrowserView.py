...
from PyQt6.QtCore import pyqtSignal, QModelIndex, Qt, QUrl
from PyQt6.QtGui import QFileSystemModel, QMouseEvent, QAction, QDesktopServices
from PyQt6.QtWidgets import QTreeView, QFileIconProvider, QMenu, QMessageBox, QInputDialog, QLineEdit
...
class FolderBrowserView(QTreeView):
    """Custom Tree View that has the ability to unselect items when clicked on an empty area"""
    open_new_tab = pyqtSignal(str)
    file_renamed_from_browser_view = pyqtSignal(str, str)

    def __init__(self, parent = None):
        super().__init__(parent)

        self.doubleClicked.connect(self._handle_tree_view_double_clicked)
        self.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        self.customContextMenuRequested.connect(self._handle_view_context_menu)
        self.doubleClicked.connect(self._handle_tree_view_double_clicked)

        self._selected_game_info : PyWrightGameInfo | None = None

    def select_pywright_game(self, selected_game_info: PyWrightGameInfo):
        ...
        self.setModel(fs_model)
        self.setRootIndex(fs_model.setRootPath(str(selected_game_info.game_path)))
    ...
    def _handle_add_new_case_action(self):
        add_dialog = CasePropertiesEditorDialog(self._selected_game_info, None, self)

        if add_dialog.exec():
            self._selected_game_info.create_new_case(add_dialog.get_case())
    ...
    def _handle_open_dir_action(self):
        if len(self.selectedIndexes()) == 0:
            ...
            QDesktopServices.openUrl(QUrl.fromLocalFile(str(self._selected_game_info.game_path)))
            return
